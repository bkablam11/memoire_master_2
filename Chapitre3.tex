...
\end{madefinition}
\begin{maremarque}
	Soit $\Re$ la relation $f \Re g \Leftrightarrow$ $f$ est une $\alpha$-réduction de $g$. Notons que cette relation est une relation d'ordre.	
\end{maremarque}
\begin{proof}
	contenu...
\end{proof}
...
\begin{maremarque}
	\begin{enumerate}
		...
		\end{itemize}
		\item[(5)] Soient $\varphi \in \mathbb{F}(M)$ et $I$ un idéal de $A$. Alors $\varphi$ est $I-bonne$ $\Longleftrightarrow$ $\varphi$ est $f_{I}-bonne$, où $f_{I}$ est la filtration $I-adic$
	\end{enumerate}
\end{maremarque}
\begin{maproposition}
	Toute filtration $f-fine$ $\varphi$ de $M$ est $f-bonne$
\end{maproposition}
\section{Quelques propositions sur les filtrations f-bonnes}
\begin{maproposition}
...
%		\item[(ii)] $J_n^2 = I_nJ_n, \quad \forall n \in \mathbb{N}$ 
%		\item[(iii)] $I_n$ est une réduction de $J_n,\quad \forall n \in \mathbb{N}$
%		\item[(iv)] $\exists k \geqslant 1$ un entier tel que $\forall n \geqslant s,$\\$ J_{s+n} = J_sJ_n,\quad I_{s+n} = I_sI_n, \quad J_s^2=I_sJ_s, J_{s+p}I_s=I_{s+p}J_s, \quad \forall \quad p =1,2,\cdots, s-1 $.
%		\item[(v)] $\exists k \geqslant 1$ un entier tel que $g^{(k)}$ est $I_k-bonne $
%		\item[(vi)] $\exists r \geqslant 1$ un entier tel que $f^{(r)}$ est une réduction de $g^{(r)}$
%		\item[(vii)] $\forall m \geqslant 1$ entier $f^{(m)}$ est une réduction de $g^{(m)}$
%		\item[(viii)] $g$ est intégrale sur $f$
%		\item[(xi)] $g$ est fortement intégrale sur $f$
%		\item[(x)] $g$ est $f-fine$
%		\item[(xi)] $g$ est $f-bonne$
%		\item[(xii)] $g$ est faiblement $f-bonne$
%		\item[(xiii)]  $\exists N \geqslant 1$ un entier tel que $t_Ng \leqslant f \leqslant g$.
%		\item[(xiv)]$\exists N \geqslant 1$ un entier tel que $t_Ng'=t_Nf'$ où $f'$ est la clôture intégrale de $f$. 
%		\item[(xv)]  $P(f) = P(g)$ où $P(f)$ est la clôture pruférienne de $f.$
%	\end{enumerate}
%\end{montheoreme}
%\begin{maproposition}
%	Sous les hypothèses du théorème \eqref{th1} en admettant que $A$ est noethérien
%	et $f$ est noethérienne alors:
%	(xi) $\Longleftrightarrow $ (ix) $\Longleftrightarrow $ (x) $\Longleftrightarrow $ (xii) $\Longleftrightarrow $ (xiii). 
%\end{maproposition}
%\begin{proof}
%	contenu...
%\end{proof}
